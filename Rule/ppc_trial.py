...
"""
Routines for interrogating the trial structure of PPC datasets.

There is some vestigial code here from previous analyses. 
"""
...
class Trial():
    ...
    def toStrings(self,recurse=True):
        ...
        s.append('(No previous trials)' if self.previous is None\
            else 'Previous trial: %d'%(self.previous.number))
        return s
...
def get_basic_trial_info(animal,session,
    ...
    for i,(istart,istop) in enumerate(zip(starts,stops)):
        ...
        if newtrial.correct:
            ...
            newtrial.rindex  = np.min(rindex) if len(rindex>1) else rindex
        ...
        # Convert (2,3)=(left,right) to (0,1)=(left,right)
        ct = c[istart+JITTER:istop-JITTER]
        cue=np.unique(ct)
        if (len(cue)!=1):
            if verbose: sys.stderr.write(' Trial %d has ambiguous cue labels\n'%i)
            ...
            if (nrc>nlc)==behavior_cue:
                if verbose: sys.stderr.write(' Infer cue from behavior and reward signals\n')
                newtrial.cue = Trial.CUE_RIGHT if behavior_cue else Trial.CUE_LEFT
            else:
                if verbose: sys.stderr.write(' Behavior disagrees with cue labels!!!\n')
            ...
            if   cue[0]==Trial.CUE_LEFT_DATASET:
                ...
                if verbose: sys.stderr.write(' Bad cue label %s, trial %d!\n'%(cue,i))
                newtrial.cue = Trial.CUE_UNKNOWN
...
def align_trials(animal,sessions,y,PSTIMERES = 1000,CUE=None,PREV=None):
    '''
    Aligns a timeseries to trials.
    ( I forget the details! Documentation Documentation Documentation )
    ...
    '''
    y = np.array(y)
    NSESSIONS,NVARS,NTIMES = y.shape
    if NVARS>NTIMES:
        raise ValueError('Signal input appears transposed; should be Nvariables x Ntimepoints')
    PDT      = 100/PSTIMERES
    VXMAX    = Trial.YSCALE + Trial.XSCALE
    x_sample = np.linspace(0,VXMAX,1000)
    t_sample = np.linspace(0,1,PSTIMERES)
    select_trials = [get_trials_with_context(animal,s) for s in sessions]
    LEFT  = 0
    RIGHT = 1
    if not CUE is None:
        select_trials = [[t for t in ts if t.cue==CUE]
                            for ts in select_trials]
    if not PREV is None:
        select_trials = [[t for t in ts if t.previous.cue==PREV]
                            for ts in select_trials]
    # Get X and Y data from each session

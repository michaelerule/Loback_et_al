...
DOING THE FIRST SET OF RESULTS FOR THE PRINTOUT. THIS MAY BE ANGLE OR VELOCITY. DON'T KNOW YET
"""
...
function make_mice(data)
    ...
    mouse_1 = v3Info("mouse_1", size(data["mouse1_day1"])[2] , 101   , 1-0.5079  ,   1 - 0.6683, data["mouse1_day1"])
    mouse_3 = v3Info("mouse_3", size(data["mouse3_day1"])[2] , 114   , 1-0.6966  ,   1 - 0.8352, data["mouse3_day1"])
    mouse_4 = v3Info("mouse_4", size(data["mouse4_day1"])[2] , 146   , 1-0.3831  ,   1 - 0.5103, data["mouse4_day1"])
    mouse_5 = v3Info("mouse_5", size(data["mouse5_day1"])[2] , 112   , 1-0.5176  ,   1 - 0.7385, data["mouse5_day1"])
...
y = ones(3)
title = Plots.scatter(y, marker=0,markeralpha=0, annotations=(2, y[2], Plots.text("Horizontal position")),axis=false, leg=false,size=(200,100))

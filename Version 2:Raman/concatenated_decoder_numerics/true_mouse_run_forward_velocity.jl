...
FORWARD VELOCITY. NOT FILLED IN YET!
"""
...
function make_mice(data)
    ...
    mouse_1 = v3Info("mouse_1", size(data["mouse1_day1"])[2] , 101   , 1-0.3646  ,   1 - 0.4391, data["mouse1_day1"])
    mouse_3 = v3Info("mouse_3", size(data["mouse3_day1"])[2] , 114   , 1-0.3214  ,   1 - 0.4965, data["mouse3_day1"])
    mouse_4 = v3Info("mouse_4", size(data["mouse4_day1"])[2] , 146   , 1-0.2116  ,   1 - 0.3543, data["mouse4_day1"])
    mouse_5 = v3Info("mouse_5", size(data["mouse5_day1"])[2] , 112   , 1-0.4339  ,   1 - 0.6695, data["mouse5_day1"])
...
y = ones(3)
title = Plots.scatter(y, marker=0,markeralpha=0, annotations=(2, y[2], Plots.text("Forward velocity")),axis=false, leg=false,size=(200,100))
